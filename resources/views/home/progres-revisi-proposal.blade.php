@include('layout.header')
											...
											<table class="table align-middle table-row-dashed fs-6 gy-5">
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        <td>{{ $alur->urutan }}</td>
                                                        <td>{{ $alur->progress }}</td>
                                                        <td><select id="dosen-select-{{ $alur->id }}" class="form-control">
                                                            <option value="{{$user->dosen_pa}}">Dosen PA</option>
                                                            <option value="{{$user->dosen_penguji}}">Dosen Penguji 1</option>
                                                            <option value="{{$user->dosen_penguji2}}">Dosen Penguji 2</option>
                                                        </select></td>
                                                        {{-- <td>{{ $alur->penanggung_jawab }}</td> --}}
                                                        <td><button {{$disabled}} class="btn btn-{{$button}} ajukan-selesai" data-id="{{ $alur->id }}" onclick="updateData({{ $alur->id }})">{{$text}}</button></td>
                                                    </tr>
        ...
        <script>
            function updateData(id) {
                const dosenPa = $('#dosen-select-' + id).val();
                const dosenPenguji1 = $('#dosen-select-' + id + ' option:eq(1)').val();
                const dosenPenguji2 = $('#dosen-select-' + id + ' option:eq(2)').val();
                let userId = '{{ session("id_user2") }}';
        
                // Show SweetAlert confirmation
                Swal.fire({
                    title: 'Konfirmasi',
                    text: 'Apakah Anda yakin ingin mengajukan revisi ini?',
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '{{ route('ajukanSelesaiRevisi') }}',
                            method: 'POST',
                            data: {
                                id: id,
                                id_user: userId,
                                dosen_pa: dosenPa,
                                dosen_penguji: dosenPenguji1,
                                dosen_penguji2: dosenPenguji2,
